...
#' Track Progress on an Indicator Over Time (with historical speed option)
#'
#' Evaluates countries’ progress in an indicator over a specified time period.
#' Progress scores can be calculated using 'speed of progress' and/or 'percentile score'.
#' Historical speeds can now be included using "his" in sequence_speed.
#'
...
#' @param scores_his Optional: precomputed historical speed scores for use when `"his"` is in `sequence_speed`.
#'
#' @return An (invisible) list containing:
#'   \itemize{
#'     \item{data_model}{The indicator data used to fit the model.}
#'     \item{predicted_changes}{Predicted changes: median, percentile, and path_speed.}
#'     \item{path_historical}{Historical paths at various speeds and percentiles.}
#'     \item{future_path}{Projected future paths; NULL if `future=FALSE`.}
#'     \item{scores}{Progress scores over the evaluation period.}
#'   }
#' @export
